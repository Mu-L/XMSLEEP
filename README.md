...
### v2.1.1 (2024-12-14)
- 🎧 **蓝牙支持**：蓝牙耳机断开时自动暂停所有音频（包括本地音频文件）
- 🔄 **最近播放**：应用重启时显示最近播放弹窗，支持快速恢复上次播放
- 🌓 **启动优化**：开屏页面适配深色/浅色模式，图标背景色自动适配
- ✨ **动画优化**：最近播放弹窗动画更自然（出现时弹性缩放，消失时渐隐）
- ⚡ **性能优化**：长音频文件播放响应速度优化，点击后立即反馈
- 🐛 **Bug修复**：修复本地音频文件无法恢复播放的问题

### v2.1.0 (2024-12-12)
- 📚 **文档优化**：更新音频资源上传指南和项目文档
- 🧹 **代码清理**：清理冗余文件，优化项目结构
- 🔧 **构建优化**：改进构建配置和依赖管理
